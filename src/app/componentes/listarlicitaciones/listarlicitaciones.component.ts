...
declare global {
  interface Window {
    electronAPI?: {
      // Usamos '?' porque esta API solo existirá en Electron
      openFolder: (folderPath: string) => void;
      onOpenFolderReply: (
        callback: (args: { success: boolean; error?: string }) => void
      ) => void; // Para escuchar la respuesta
    };
  }
}
...
export class ListarlicitacionesComponent implements OnInit {
  ...
  async openFolder(folderPath: string | null | undefined) {
    ...
    // >>> Lógica para usar la API expuesta por el preload script <<<
    // Verificar si la API 'electronAPI' existe en el objeto global 'window'.
    // Esta API solo existirá si el preload script se ha ejecutado correctamente (en Electron).
    if (window.electronAPI && window.electronAPI.openFolder) {
      console.log(`Llamando a electronAPI.openFolder con ruta: ${folderPath}`);
      // Llamar a la función expuesta por el preload script
      window.electronAPI.openFolder(folderPath);

      // La respuesta (éxito/error) se manejará en el listener configurado en ngOnInit
      // No necesitamos lógica de éxito/error aquí directamente después de la llamada.
    } else {
