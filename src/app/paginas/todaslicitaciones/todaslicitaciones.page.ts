  ...
  ngOnInit() {
    ...
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      if (clienteDesdeStats) {
        this.busquedaCliente = clienteDesdeStats; // Opcional: Limpia el query param después de usarlo si no quieres que se quede en la URL // this.router.navigate([], { queryParams: { cliente: null }, queryParamsHandling: 'merge' });
      } else {
        // Si no hay parámetro de cliente, inicializa busquedaCliente como vacío
        this.busquedaCliente = '';
      } // Inicializa otros filtros aquí si es necesario // (aunque tu código original los inicializaba en onFiltrosChanged o al declarar)
